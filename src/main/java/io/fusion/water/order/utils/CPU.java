...
public class CPU {
	...
	public static String printCpuStats() {
		...
		String val = "";
		try  {	val = String.format("%.02f", CPU.getProcessCpuLoad()); } catch (Exception e) { val = ""; }
		sb.append("|PCPU=").append(val);
		try  {	val = String.format("%.02f", CPU.getSystemCpuLoad()); } catch (Exception e) { val = ""; }
		sb.append("|SCPU=").append(val);

		sb.append("|FM=").append(toMBString(freeMemory()));
		sb.append("|TM=").append(toMBString(totalMemory()));

		try  {	val = String.format("%.02f", CPU.getFreePhysicalMemorySize()); } catch (Exception e) { val = ""; }
